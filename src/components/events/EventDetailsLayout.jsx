...
const EventDetailsLayout = ({
  ...
  /*
   * Keep checking the event end time while the event page is open.
   * This means the registration section disappears automatically
   * once the event has ended, without requiring a page refresh.
   */
  useEffect(() => {
    if (!event) {
    ...
    const checkIfEventHasEnded = () => {
      ...
      if (Number.isNaN(eventEndTime)) {
        setEventHasEnded(false);
        return;
      }

      setEventHasEnded(eventEndTime <= Date.now());
    };

    checkIfEventHasEnded();

    const timer = setInterval(checkIfEventHasEnded, 30 * 1000);
      ...
      <div className="flex flex-1 flex-col overflow-y-auto px-6 pb-20 pt-8 sm:px-12">
        <div className="relative flex justify-center">
          <EventCover
            ...
            className="h-auto w-auto max-w-full sm:w-110 object-contain"
          />
        </div>
